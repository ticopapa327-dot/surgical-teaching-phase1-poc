...
现场保存的诊断快照 JSON 可用 `node scripts/summarize-diagnostics.cjs snapshot-a.json snapshot-b.json > diagnostics.csv` 汇总为 CSV，输出包含会话 ID、媒体房间 ID、视频码率、包计数、RTT 和 ICE 路由。
