...
- 信令服务支持可选共享令牌门禁，可通过环境变量启用；该能力只用于 PoC 入口保护，不替代生产级鉴权。
- 信令呼叫具备忙线保护和待处理超时，避免未确认呼叫长期占用终端状态。
...
默认监听 `ws://127.0.0.1:7077/signal`，健康检查地址为 `http://127.0.0.1:7077/health`，在线会话摘要地址为 `http://127.0.0.1:7077/sessions`。
...
`test:signaling` 启动本地 WebSocket 信令服务器，模拟手术室端、示教室端和观摩端，验证注册、通讯录、会话目录、呼叫、接受、默认通道、订阅和参与上限拒绝。
...
- 信令服务器当前只做控制面验证；可选共享令牌不等同于生产级鉴权，仍不做用户级权限、持久化、TLS、审计和媒体转发。
