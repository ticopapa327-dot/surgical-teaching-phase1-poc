...
- 信令服务提供内存型控制面事件日志，可通过 HTTP 查询最近事件摘要；该日志用于调试和验收追踪，不替代生产审计。
...
- 手术室端可在已建立信令会话后发布通道 1 视频媒体。
- 示教室端可通过既有 `peer.signal` 透传完成 WebRTC offer、answer 和 ICE 协商。
- 示教室端远端显示区优先显示收到的真实远端 MediaStream，不再只能依赖本地模拟预览流。
- 当前只验证两端 P2P 单路视频链路，不包含 SFU、多方转发、TURN 中继、RTSP/SRT 接入、远端音频通话和手机直播分发。

## 技术边界

本工程采用 Electron + React + MediaRecorder 实现，适合快速验证 USB 采集卡是否以标准摄像头设备暴露、四画面预览是否稳定、选择性录制流程是否成立，以及呼叫/订阅/布局控制流是否合理。

当前阶段 3 已可通过浏览器 WebRTC P2P 建立通道 1 的真实远端视频链路，但它不等同于生产级媒体服务或 SFU。真实跨网段、多人会议、Android 会议平板、远端音频、手机直播和弱网恢复必须在后续阶段接入媒体服务后重新验证。
...
`test:smoke` 使用系统 Chrome 打开 Vite 页面，验证页面渲染、模拟预览、呼叫确认、多路拉取、布局切换、标注、参与上限提示、前端连接真实 WebSocket 信令服务器后进入已接受会话，以及双页面 WebRTC 单路视频链路。
...
- [Phase 3 单路 WebRTC 媒体 PoC 说明](docs/phase3-media-poc.md)
...
- 阶段 3 当前只验证两端 P2P 单路视频，不包含真实 SFU、远端网络抖动、TURN 中继、远端音频和 Android 平板端原生能力。
