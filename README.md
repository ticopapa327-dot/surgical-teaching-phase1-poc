...
- 当前只验证两端 P2P 单路视频和基础音频链路，不包含 SFU、多方转发、TURN 中继、RTSP/SRT 接入和手机直播分发。

## 技术边界

本工程采用 Electron + React + MediaRecorder 实现，适合快速验证 USB 采集卡是否以标准摄像头设备暴露、四画面预览是否稳定、选择性录制流程是否成立，以及呼叫/订阅/布局控制流是否合理。

当前阶段 3 已可通过浏览器 WebRTC P2P 建立通道 1 的真实远端视频链路，并在交互模式下承载基础远端音频，但它不等同于生产级媒体服务或 SFU。真实跨网段、多人会议、Android 会议平板、手机直播和弱网恢复必须在后续阶段接入媒体服务后重新验证。
...
`test:smoke` 使用系统 Chrome 打开 Vite 页面，验证页面渲染、模拟预览、呼叫确认、多路拉取、布局切换、标注、参与上限提示、前端连接真实 WebSocket 信令服务器后进入已接受会话，以及双页面 WebRTC 单路视频和音频链路。
...
- 阶段 3 当前只验证两端 P2P 单路视频和基础音频，不包含真实 SFU、远端网络抖动、TURN 中继和 Android 平板端原生能力。
