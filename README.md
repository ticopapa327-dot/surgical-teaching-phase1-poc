...
- 发布中远端订阅发生变化时，手术室端会按新的订阅集合自动重新协商媒体链路。
...
- 当前只验证两端浏览器 P2P 按订阅多路视频和基础音频链路，不包含 SFU、多方转发、TURN 中继、RTSP/SRT 接入和手机直播分发。
...
- 阶段 3 当前只验证两端 P2P 按订阅多路视频和基础音频，不包含真实 SFU、远端网络抖动、TURN 中继和 Android 平板端原生能力。
