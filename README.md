...
- 信令服务提供轻量会话目录摘要，用于观摩端发现可加入会议；会话包含 `mediaRoomId`，用于后续 SFU 或媒体服务建立房间映射；该目录不包含标注内容、订阅细节、患者信息或媒体数据。
...
- 信令服务支持会话内 `peer.signal` 透传，用于后续 WebRTC offer、answer 和 ICE 候选协商；透传消息带有 `mediaRoomId`，便于把控制面事件和后续媒体服务房间对应起来。
...
- 示教室端可通过既有 `peer.signal` 透传完成 WebRTC offer、answer 和 ICE 协商，事件日志会保留会话 ID、媒体房间 ID、通道和轨道数摘要。
...
- 提供“复制诊断快照”，输出浏览器运行环境、控制面、通道、音频、媒体、结构化 WebRTC 指标、PeerConnection 和最近 10 条信令事件摘要；快照不包含信令令牌、设备 ID、患者信息、SDP、ICE candidate 原文或媒体数据。
...
117 远程浏览器控制推荐使用 SSH 隧道命令：`npm run test:remote:signal:tunnel`、`npm run test:remote:media:tunnel` 和 `npm run test:remote:audio:tunnel` 会自动启动 PC-B Edge DevTools、转发到 PC-A 本机 `127.0.0.1:9224`，并在测试后清理隧道。
