...
| Electron 主进程 | 本地录像文件写入、索引、定位、导出、环境变量配置的 FTP 上传 | HIS、生产级文件归档策略、AI 服务调用 |
...
当前 PoC 已验证手术室端可按远端订阅，通过 `peer.signal` 完成少量浏览器 WebRTC P2P 多通道视频协商、显示和基础重协商。该能力仍不是生产媒体服务：真实 USB 4 路长稳运行、延迟、码率、同步、丢包恢复、回声消除、大规模多方会议、TURN 中继和 SFU 转发必须另行验证。
