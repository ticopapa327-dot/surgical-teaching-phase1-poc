...
`/sessions` 返回当前在线信令会话摘要，只包含会话 ID、媒体房间 ID、模式、参与上限、参与端 ID、参与人数和开始时间，不包含标注内容、通道订阅详情、患者信息或媒体数据。
...
注册字段会被服务端归一化：`endpointId`、`name` 必须是非空字符串，否则使用安全默认值；`address` 若为空、非法或为 loopback 地址，则回退为 WebSocket 连接的远端地址；`role` 仅接受 `operating-room`、`teaching-room`、`observer`，非法值回落为 `observer`；`capabilities` 会过滤非字符串、空值和重复值；`channels` 只接受对象项，并对 `id`、`label`、`role` 做默认值和长度限制。
...
客户端应据此恢复会话状态。服务端恢复只覆盖控制面状态，不直接承载音视频；当前前端会在远端 endpoint 注册版本变化后，由手术室端自动重新发起 WebRTC 协商以恢复媒体链路。
...
服务端向双方发送 `session.started`。会话中包含 `mediaRoomId`，用于后续 SFU 或媒体服务将信令会话映射到媒体房间；当前 PoC 仍只透传浏览器 P2P 协商消息，不由信令服务转发媒体。事件日志会记录 `call.accepted` 和 `session.started`，其中 `call.accepted` 只包含呼叫 ID、会话 ID、媒体房间 ID、接受端、最终模式和参与上限摘要。默认订阅通道为 `ch1`。
...
2. TLS、证书管理和跨网段安全接入。
3. 会话持久化、服务端高可用和生产级重连恢复；当前只提供在线端点同 ID 重注册后的控制面会话恢复。
4. 生产级媒体服务、SFU 转发、SRT/RTSP 接入和手机直播分发；当前只透传浏览器 WebRTC P2P 协商消息，不转发媒体。
5. 标注权限控制、版本控制和录像回放绑定。
6. HIS 患者信息绑定、录像文件索引和 AI 处理任务分发。

后续进入真实媒体服务阶段时，信令协议应扩展媒体发布、媒体订阅或 SFU 房间控制字段，但控制面仍不应直接承载媒体数据。
