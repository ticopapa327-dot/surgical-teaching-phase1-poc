...
117 Windows 远程浏览器已支持本地自动化媒体 smoke：PC-A 执行 `npm run test:remote:media:tunnel` 可通过 SSH 隧道启动并控制 117 的 Edge DevTools，完成 4 路订阅、发布媒体、远端 live 检查、双端快照保存和诊断分析。媒体 smoke 通过后，执行 `npm run test:remote:diagnostics` 会自动读取最近一组 118/117 快照，生成 `*-diagnostics.csv` 并再次运行带门禁的诊断分析；也可通过 `npm run test:remote:diagnostics -- snapshot-a.json snapshot-b.json` 分析指定快照。交互音频接收专项可执行 `npm run test:remote:audio:tunnel`，自动验证 118 本地音频轨道能送达 117；由于 117 局域网 HTTP 页面不是安全上下文，该测试不要求 117 采集本地麦克风。
